---
title: Передача DBS-заказа в доставку
api: uzum-seller
method: POST
path: /v1/dbs/order/{orderId}/delivering
operation_id: deliveringDbsOrder
tags:
  - Работа с заказами FBS/DBS
spec_version: 1.0.0
source: "https://api-seller.uzum.uz/api/seller-openapi/swagger/swagger-ui/swagger-ui/index.html"
deprecated: false
content_sha: 6b8483e58a3b0fab
---

# Передача DBS-заказа в доставку

`POST /v1/dbs/order/{orderId}/delivering`

Переводит DBS-заказ в статус DELIVERING.

## Параметры

| Имя | Где | Тип | Обяз. | Описание |
|---|---|---|---|---|
| `orderId` | path | integer<int64> | да | Идентификатор заказа, который необходимо перевести в доставку. |
...
**400** — Ошибки выполнения: - `seller-order-01`: заказ не найден; - `seller-order-02`: неподходящий статус (разрешено только для PACKING); - `seller-order-03`: истёк срок доставки; - `seller-order-15`: не заполнены идентификаторы; - `fbs-18-invalid-order-type`: разрешено только для DBS-заказов.

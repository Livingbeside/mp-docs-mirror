---
title: Подтверждение выдачи DBS-заказа
api: uzum-seller
method: POST
path: /v1/dbs/order/{orderId}/completed
operation_id: completedDbsOrder
tags:
  - Работа с заказами FBS/DBS
spec_version: 1.0.0
source: "https://api-seller.uzum.uz/api/seller-openapi/swagger/swagger-ui/swagger-ui/index.html"
deprecated: false
content_sha: 592889fa34597e02
---

# Подтверждение выдачи DBS-заказа

`POST /v1/dbs/order/{orderId}/completed`

Переводит DBS-заказ в статус COMPLETED после выдачи покупателю.

## Параметры

| Имя | Где | Тип | Обяз. | Описание |
|---|---|---|---|---|
| `orderId` | path | integer<int64> | да | Идентификатор выдаваемого заказа. |
| `issueCode` | query | integer<int64> | нет | Код выдачи заказа (если требуется подтверждение покупателя). |
...
**400** — Ошибки выполнения: - `seller-order-01`: заказ не найден; - `seller-order-02`: неподходящий статус (разрешено только для DELIVERING); - `seller-order-03`: истёк срок доставки; - `fbs-18-invalid-order-type`: разрешено только для DBS-заказов; - `seller-order-37`: код подтверждения заказа не найден; - `seller-order-38`: неверный код подтверждения; - `seller-order-39`: неверный код подтверждения, повторите попытку позже.

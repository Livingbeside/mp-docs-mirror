...
- `dates` — object **обязательный**. Информация о новой дате доставки заказа.
  - `toDate` — string<date> **обязательный**. Новая дата доставки заказа. Формат даты: `ГГГГ-ММ-ДД`.
- `reason` — string (USER_MOVED_DELIVERY_DATES, PARTNER_MOVED_DELIVERY_DATES) **обязательный**. Причина переноса доставки заказа.

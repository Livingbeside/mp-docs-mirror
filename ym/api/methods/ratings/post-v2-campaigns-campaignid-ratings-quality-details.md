...
- `result` — object. Информация о заказах, которые повлияли на индекс качества.
  - `affectedOrders` — array[object] **обязательный**. Список заказов, которые повлияли на индекс качества.
    - `orderId` — integer<int64> **обязательный**. Идентификатор заказа.
    - `description` — string **обязательный**. Описание проблемы.
    - `componentType` — string (DBS_CANCELLATION_RATE, DBS_LATE_DELIVERY_RATE, FBS_CANCELLATION_RATE, FBS_LATE_SHIP_RATE) **обязательный**. Тип составляющей индекса качества.

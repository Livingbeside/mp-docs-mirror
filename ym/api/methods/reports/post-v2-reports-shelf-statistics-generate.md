...
- `dateFrom` — string<date> **обязательный**. Начало периода, включительно.
- `dateTo` — string<date> **обязательный**. Конец периода, включительно.
- `attributionType` — string (CLICKS, SHOWS) **обязательный**. Тип атрибуции.

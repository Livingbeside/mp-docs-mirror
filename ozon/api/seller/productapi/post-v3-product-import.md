...
Для каждого товара вы можете загрузить до 50 изображений, включая главное.
Если передать значение `primary_image`, максимальное количество изображений в `images` — 49.
Если параметр `primary_image` пустой, то в `images` можно передать до 50 изображений.
...
- `items` — array[object]. Массив данных.
  ...
  - `images` — array[string]. Массив изображений. Изображения показываются на сайте в таком же порядке, как в массиве. Если не передать значение `primary_image`, первое изображение в массиве будет главным для товара. Если вы передали значение `primary_image`, передайте до 49 изображений. Если параметр `primary_image` пустой, передайте до 50 изображений. Формат: адрес ссылки на изображение в общедоступном облачном хранилище. Формат изображения по ссылке — JPG или PNG.
  ...
  - `price` — string **обязательный**. Предельная цена товара без акций. Выше этого значения цену для покупателя не поднимем.

---
title: Создать сертификат качества
api: ozon-seller
method: POST
path: /v2/product/certificate/create
operation_id: ProductCertificateCreate
tags:
  - BetaMethod
spec_version: 2.1
source: "https://docs.ozon.ru/api/seller/"
deprecated: false
content_sha: d2add8be2e32e717
---
...
- `params` — array[object]. Информация о параметрах в запросе.
  - `error` — string. Ошибки в параметре для `state = INVALID`.
  ...
  - `state` — string (VALID, INVALID, MISSING). Состояние параметра: - `VALID` — заполнен корректно; - `INVALID` — заполнен некорректно; - `MISSING` — не заполнен.
- `status` — string (INCOMPLETE, COMPLETED). Статус сертификата: - `INCOMPLETE` — не загружен, некоторые параметры переданы некорректно; - `COMPLETED` — загружен.

---
title: Получить список необработанных отправлений
api: ozon-seller
method: POST
path: /v4/posting/fbs/unfulfilled/list
operation_id: PostingFbsUnfulfilledList
tags:
  - FBS
spec_version: 2.1
...
- `filter` — object. Фильтр запроса. Используйте фильтр по времени сборки — `cutoff` или по дате передачи отправления в доставку — `delivering_date`. Если использовать их вместе, в ответе вернётся ошибка. Чтобы использовать фильтр по времени сборки, заполните поля `cutoff_from` и `cutoff_to`. Чтобы использовать фильтр по дате передачи отправления в доставку, заполните поля `delivering_date_from` и `delivering_date_to`.
  - `cutoff_from` — string<date-time>. Время, до которого продавцу нужно собрать заказ. Начало периода.
  - `cutoff_to` — string<date-time>. Время, до которого продавцу нужно собрать заказ. Конец периода.
  ...
  - `statuses` — array[string]. Статус отправления: - `acceptance_in_progress` — идёт приёмка; - `awaiting_approve` — ожидает подтверждения; - `awaiting_packaging` — ожидает упаковки; - `awaiting_registration` — ожидает регистрации; - `awaiting_deliver` — ожидает отгрузки; - `arbitration` — арбитраж; - `client_arbitration` — клиентский арбитраж доставки; - `delivering` — доставляется; - `driver_pickup` — у водителя; - `not_accepted` — не принято на сортировочном центре.
...
- `postings` — array[object]. Список отправлений.
  ...
  - `tariffication_steps` — array[object]. Этапы тарификации.
    ...
    - `tariff_charge` — object. Скидка или надбавка.
      - `amount` — string. Сумма.
      - `currency` — string. Валюта.
    - `tariff_deadline_at` — string<date-time>. Дата и время окончания этапа тарификации. После этой даты автоматически применяется следующий этап.

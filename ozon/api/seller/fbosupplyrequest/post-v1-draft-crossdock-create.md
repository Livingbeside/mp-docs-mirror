---
title: Создать черновик заявки на поставку кросс-докингом
api: ozon-seller
method: POST
path: /v1/draft/crossdock/create
operation_id: DraftCrossdockCreate
tags:
  - FboSupplyRequest
spec_version: 2.1
source: "https://docs.ozon.ru/api/seller/"
deprecated: false
content_sha: 676d6a0cd9b572b9
---
...
Черновик заявки на поставку не отображается в личном кабинете продавца и доступен 30 минут.
...
- `delivery_info` — object **обязательный**. Информация о доставке.
  - `drop_off_warehouse` — object. Информация о пункте отгрузки. Если `type = DROPOFF`, параметр обязательный.
    - `warehouse_id` — integer<int64> **обязательный**. Идентификатор склада.
    - `warehouse_type` — string (DELIVERY_POINT, SORTING_CENTER, CROSS_DOCK, ORDERS_RECEIVING_POINT, FULL_FILLMENT) **обязательный**. Тип склада, пункта выдачи заказов или сортировочного центра: - `DELIVERY_POINT` — пункт выдачи заказов; - `SORTING_CENTER` — сортировочный центр; - `CROSS_DOCK` — кросс-докинг; - `ORDERS_RECEIVING_POINT` — специализированный пункт приёма заказов; - `FULL_FILLMENT` — фулфилмент. По умолчанию: `DELIVERY_POINT`.
  - `seller_warehouse_id` — integer<int64>. Идентификатор склада продавца. Если `type = PICKUP`, параметр обязательный. Получите значение параметра методом [/v1/warehouse/fbo/seller/list](#operation/WarehouseFboSellerList).
  - `type` — string (DROPOFF, PICKUP) **обязательный**. Тип поставки: - `DROPOFF` — отгрузка заказов в пункт приёма; - `PICKUP`— отгрузка заказов курьеру. По умолчанию: `DROPOFF`.

---
title: Создать черновик заявки на поставку для нескольких кластеров
api: ozon-seller
method: POST
path: /v1/draft/multi-cluster/create
operation_id: DraftMultiClusterCreate
tags:
  - FboSupplyRequest
spec_version: 2.1
source: "https://docs.ozon.ru/api/seller/"
deprecated: false
content_sha: 7da6d32e5e9d54aa
---
...
- `clusters_info` — array[object] **обязательный**. Информация о кластерах.
...
- `delivery_info` — object **обязательный**. Информация о доставке.
  - `drop_off_warehouse` — object. Информация о пункте отгрузки.
    - `warehouse_id` — integer<int64> **обязательный**. Идентификатор склада.
    - `warehouse_type` — string (DELIVERY_POINT, SORTING_CENTER, CROSS_DOCK, ORDERS_RECEIVING_POINT, FULL_FILLMENT) **обязательный**. Тип пункта отгрузки: - `DELIVERY_POINT` — пункт выдачи заказов; - `SORTING_CENTER` — сортировочный центр; - `CROSS_DOCK` — кросс-докинг; - `ORDERS_RECEIVING_POINT` — пункт приёма заказов; - `FULL_FILLMENT` — фулфилмент. По умолчанию: `DELIVERY_POINT`.
  - `seller_warehouse_id` — integer<int64>. Идентификатор склада продавца.
  - `type` — string (DROPOFF, PICKUP) **обязательный**. Тип поставки: - `DROPOFF` — доставка до drop-off пункта; - `PICKUP` — доставка курьером от склада продавца. По умолчанию: `DROPOFF`.
